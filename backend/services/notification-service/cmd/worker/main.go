...
import (
	...
	"github.com/nihfery/takein/services/notification-service/internal/consumer"
)
...
func run() error {
	runtime, err := config.LoadRuntime("notification-worker")
	if err != nil {
		return err
	}
	...
	client, err := kafka.Open(ctx, kafka.Config{Brokers: runtime.KafkaBrokers, ClientID: runtime.ServiceName, ConsumerGroup: "takein-notification-domain-events-v1", Topics: []string{"takein.booking.events.v1", "takein.payment.events.v1", "takein.billing.events.v1", "takein.provider.events.v1"}, MaxRetries: runtime.KafkaMaxRetries, SecurityProtocol: runtime.KafkaSecurityProtocol, SASLMechanism: runtime.KafkaSASLMechanism, Username: runtime.KafkaUsername, Password: runtime.KafkaPassword, TLSCAFile: runtime.KafkaTLSCAFile, TLSServerName: runtime.KafkaTLSServerName})
	if err != nil {
		return err
	}
	defer client.Close()
	...
	go func() {
		...
		failures <- (kafkaconsumer.Runner{Client: client, Service: runtime.ServiceName, ConsumerGroup: "takein-notification-domain-events-v1", MaxRetries: 5, Handle: processor.Process}).Run(ctx)
	}()
	err = <-failures
	cancel()
